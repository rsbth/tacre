...
public class rsbh extends AgentImpl {
  ...
  private float optimalVal=300;
  private float[] highboundry;
  private float[] lowboundry;
  private boolean[] latestup; 
  
  private float costlyaccom=45;
  private float cheapaccom=45;
  private float[] triag;
  private float[] hotelearlierpris;
  
  protected void init(ArgEnumerator args) {
	  triag = new float[agent.getAuctionNo()];
	    hotelearlierpris = new float[agent.getAuctionNo()];
	    highboundry = new float[agent.getAuctionNo()];
	  prices = new float[agent.getAuctionNo()];
    
    lowboundry = new float[agent.getAuctionNo()];
    latestup =new boolean[agent.getAuctionNo()];
    for(boolean val : latestup)
    {
    	val = false;
    }
  }
  ...
  public void quoteUpdated(Quote quote) {
	...
	triag[auction]=(triag[auction]+(quote.getAskPrice()-hotelearlierpris[auction]))/2;
	
	if(prices[auction]< quote.getAskPrice()+triag[auction])
	{
		prices[auction]=quote.getAskPrice()+triag[auction];
    ...
    else if (auctionCategory == TACAgent.CAT_FLIGHT)
    {
    	...
    	int alloc = agent.getAllocation(auction);
    	lowboundry[auction]=(float)(agent.getQuote(auction).getAskPrice()-0.5*agent.getQuote(auction).getAskPrice());
    	if(alloc>0)
    	{
    		
    		if(!latestup[auction])
    		{
    			highboundry[auction]=(float)(agent.getQuote(auction).getAskPrice()+0.5*agent.getQuote(auction).getAskPrice());
    			Bid oldbid =quote.getBid();
    			Bid bid = new Bid(auction);
    			bid.addBidPoint(alloc, lowboundry[auction]);
    			agent.replaceBid(oldbid, bid);
    			latestup[auction]=true;
    		}
    		if(askPrice>highboundry[auction])
    		{
    			Bid oldbid =quote.getBid();
    			Bid bid = new Bid(auction);
